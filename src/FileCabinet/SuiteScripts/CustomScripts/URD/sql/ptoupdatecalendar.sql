SELECT 'NETSUITE-' || timeoffrequest.id AS id 
	, 1.0 AS version
	, CASE WHEN timeoffrequest.approvalstatus = 7 THEN 'update'
		WHEN timeoffrequest.approvalstatus = 8 THEN 'update'
		WHEN timeoffrequest.approvalstatus = 9 THEN 'delete'
		WHEN timeoffrequest.approvalstatus = 10 THEN 'delete'
		ELSE 'nothing'
	 END AS action
	...
	, CASE WHEN timeoffrequest.approvalstatus = 7 THEN 'planned'
		...
		WHEN timeoffrequest.approvalstatus = 9 THEN 'rejected'
		WHEN timeoffrequest.approvalstatus = 10 THEN 'cancelled'
		ELSE 'nothing'
	 END AS status
	...
	, 'This event is synchronized from NetSuite.' AS description
